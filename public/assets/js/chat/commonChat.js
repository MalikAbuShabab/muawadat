    ...
    $(document).ready(function () {
        let messageInput = $("#message_box");
        // Send message on button click
        $(document).on("click", ".send_message", async function () {
            await triggerSendMessage($(this));
        });
    
        // Send message on Enter key press
        messageInput.on("keypress", async function (event) {
            if (event.key === "Enter") {
                event.preventDefault(); // Prevent page reload
                await triggerSendMessage($(".send_message")); // Simulate button click
            }
        });
    
         // Function to check if message is empty/whitespace
        function isOnlyWhitespace(str) {
            return str.trim().length === 0;
        }
        // Function to send message
        async function triggerSendMessage(button) {
            var room_id = button.attr('data-id');
            var roomIdText = $(`#room_${room_id}`).attr('data-roomid');
            var message = $('#message_box').val();
    
            if (!room_id || isOnlyWhitespace(message)) return; 
    ...
    //     })
    // });
    $("#outer_search").on("keyup", function() {
        var value = $(this).val().toLowerCase();
        var noResult = true;
    
        $(".chatRoomsDivs").each(function() {
            var roomId = $(this).data('text') ? $(this).data('text').toString().toLowerCase() : "";
            var message = $(this).find('.orderChatMessage').text().toLowerCase();
            var username = $(this).find('.chat_name').text().toLowerCase();
    
            // Match with roomId, message, or username
        ...
        .then(async response => {
            ...
            if(response.status == 200) {
                if(response.data.length > 0) {
                    // First pass: Collect all dates with messages
                    await response.data.forEach(async function (data) {
                        var className = 'left-message';
                        var flex = '';
                        var cdate = new Date(data.created_date);
                        var currentDate = cdate.toDateString(); // Format: "Fri Mar 21 2025"
                        var timeOnly = cdate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true });
                    
                        // If it's a new date, insert the date badge before the first message of that date
                        let dateBadge = "";
                        if (lastDate !== currentDate) {
                            dateBadge = `<div class="chat-date-separator text-center my-3">
                                            <span class="badge show_chat_day p-2">${currentDate}</span>
                                         </div>`;
                        ...
                        html += `${dateBadge} <!-- Date badge now inside the chat container -->
                                 <div class="${className}">
                                    ${flex}
                                    <div class="mb-4">
                    ...
                    await response.data.userData.forEach(function (data) {
                     html+= `<div class="alPhoneNumberDetails">
                            <ul class="p-0 m-0 d-lg-flex align-items-center text-lg-left text-center">
                                ...
                                <li><span class="alUserName">${data.username}  (${data.user_type}) </span>
        ...
        .then(async response => {
            ...
            if(response.status == 200) {
                if(response.data.roomData.length > 0) {
                    ...
                    html = `<div id="noResultsMessage" class="alert alert-warning mt-2 text-center" style="display: none;">
        No users found matching your search!
    </div><div id="chatRooms_${data._id}" data-text="${data.room_id}" data-chat_type="${chatType}" data-product_id="${data.product_id!=undefined?data.product_id:''}"  data-sort="${i}" data-timestamp="" class="list-group rounded-0 chatRoomsDivs">
                        <div id="room_${data._id}" data-orderid="${data.order_id}" data-chat_type="${chatType}" data-ordervendorid="${data.order_vendor_id}" data-vendor_id="${data.vendor_id}" data-product_id="${data.product_id!=undefined?data.product_id:''}"  data-id="${data._id}" data-roomid="${data.room_id}" data-roomname="${data.room_id}" class="chat-list-item row fetchChat">
                            ...
                            <div class="col-9 position-relative pl-0">
                                <div class="alNameTime last_message">
                                <h6 id="preview_message_name_${data._id}" class="mb-1 mt-0 chat_name">${last_message_name}</h6>
