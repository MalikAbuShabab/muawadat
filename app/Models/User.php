...
class User extends Authenticatable implements Wallet, WalletFloat, Auditable
{
     ...
     */
    protected $fillable = [
        'name', 'email', 'password', 'description', 'phone_number', 'dial_code', 'image', 'is_email_verified', 'email_verified_at', 'is_phone_verified', 'type', 'status', 'device_type', 'device_token', 'country_id', 'role_id', 'auth_token', 'remember_token', 'timezone', 'import_user_id', 'last_login_at', 'is_admin', 'geo_ids', 'is_presignup', 'custom_allergic_items','front_identity_doc_verified','back_identity_doc_verified'
    ];
    ...
    }public function sellerBids()
    {
        return $this->hasMany(P2pBid::class, 'seller_id', 'id');
    }

    // A user can have multiple bids as a buyer
    public function buyerBids()
    {
        return $this->hasMany(P2pBid::class, 'buyer_id', 'id');
    }
    ...
    {
        return $this->hasOne(UserRefferal::class);
    }
