...
public function send_notification(Request $request)
	...
	public function getBrands()
    {
		try {
			if (clientPrefrenceModuleStatus('p2p_check')) {
				$brands = Brand::with('translation')->where('status',1)->get();
				return $this->successResponse($brands, 'Project Type List', 200);
			} else {
				return $this->errorResponse('Project type list not found', 500);
			}
		}catch(\Exception $e) {
			return $this->errorResponse('Exception occured', 500);
		}
	...
	// P2P Brand Categories list api
	public function brandCategories(Request $request, $brand_id) 
	{
		try {
			if (clientPrefrenceModuleStatus('p2p_check')) {
				$categories = BrandCategory::with(['brand','categoryTranslation','categoryDetail'])
					->where('brand_id', $brand_id)->get()
					->map(function($brandCategory) {
						return [
							'brand' => $brandCategory->brand,
							'category_id' => $brandCategory->category_id,
							'name' => $brandCategory->categoryTranslation->name ?? '',
							'description' => $brandCategory->categoryTranslation->description ?? '',
							'category_detail' => $brandCategory->categoryDetail?? '',
						];
					});

				if ($categories->isEmpty()) {
					return $this->errorResponse('No categories found for this brand', 404);
				}

				return $this->successResponse($categories, 'Brand categories retrieved successfully', 200);
			} else {
				return $this->errorResponse('P2P module is not enabled', 500);
			}
		} catch (\Exception $e) {
	...
	// P2P Store Company Basic Info Store API
	...
	public function storeCompanyBasicInfo(Request $request)
	{
		...
		try {
			...
			$product = Product::updateOrCreate(
				...
				[
					...
					'sku' => $slug,
					'url_slug'=>$generated_slug,
					'title' => empty($request->name) ?? '',
					'vendor_id' => $user_vendor->vendor_id ?? $user->id,
					'publish_at' => date('Y-m-d H:i:s'),
					'type_id' => 1,
					'is_live' => 0,
					'category_id'=> $request->company_type_id ?? null,
					'company_type' => $request->company_type_id ?? null,
					'company_website' =>$request->website ?? null,
					'brand_id' => $request->project_type_id ?? null,
					'company_sale_type' =>$request->project_type_id ?? null,
					'company_launch_date' =>$request->launch_date ?? null,
					'country_id' =>$request->country ?? null,
					'company_team_size' =>$request->team_size ?? null,
					'company_business_model' =>$request->business_modal ?? null,
					'description' => $request->company_description,
					'company_technology' =>$request->company_technology ?? null,
					'address'=> $locationAddress ?? null,
					'latitude'=> $locationLat ?? null,
					'longitude'=> $locationLong ?? null,
				]
			);
			...
			$datatrans[] = [
				...
				'body_html' => !empty($request->company_description)?$request->company_description:'',
				'meta_title' => '',
				'meta_keyword' => '',
				'meta_description' => '',
				'product_id' => $product->id,
			...
			$product_category = new ProductCategory();
			$product_category->product_id = $product->id;
			$product_category->category_id = $request->company_type_id;
			$product_category->save();
			$proVariant = new ProductVariant();
			...
			if ($request->has('dynamic_fields') && count($request->dynamic_fields) > 0) {
				$dynamicFields = $request->dynamic_fields;
				$pairedFields = [];
				for ($i = 0; $i < count($dynamicFields); $i++) {
					if (isset($dynamicFields[$i]['name'])) {
						$name = $dynamicFields[$i]['name'];
						if (isset($dynamicFields[$i + 1]['value'])) {
							$value = $dynamicFields[$i + 1]['value'];
							$pairedFields[] = ['name' => $name, 'value' => $value];
						}
					}
				}
				...
				// \Log::info('Dynamic fields saved successfully.', $pairedFields);
			}

			if ($product->isEmpty()) {
				return $this->errorResponse('Store Company Basic Info Not Found', 404);
			}

			return $this->successResponse($product, 'Company Basic Info store successfully', 200);
	
		} catch (\Exception $e) {
			\Log::error('Error saving product: ' . $e->getMessage());
			return $this->errorResponse('Failed to store Company Basic Info', 500);
