...
class StripeGatewayController extends BaseController
{
    ...
    public function stripePurchase(request $request)
    {
        ...
        // try {
            ...
            if ($customer = array_shift(\Stripe\Customer::search(['query' => "metadata['user_id']:'{$user->id}'"])->data)) {
            ...
            if ($customer_id) {
                ...
                $this->saveUserPaymentMethod($request);
            }
            ...
            if (! is_null($customer_id)) {
                $postdata['customerReference'] = $customer_id;
            } else {
                $postdata['source'] = $token;
            }
