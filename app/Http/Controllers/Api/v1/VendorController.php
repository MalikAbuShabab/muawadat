...
class VendorController extends BaseController{
    ...
    public function getVendorTransactions(Request $request)
    {
        try {
            ...
            $allorders = $allorders->where('vendor_id', $request->vendor_id)->whereIn('order_status_option_id', [6])->sum('subtotal_amount');
            switch ($type) {
                case 'complete':
            ...
            foreach ($orders as $order) {
                ...
                $order->total_amount = $order->orderDetail->total_amount;
                $product_details = [];
