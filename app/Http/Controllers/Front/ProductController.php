...
class ProductController extends FrontController{
    ...
    public function sendWithdrawCode(Request $request)
    {
        ...
        $existingOtp = P2pWithdrawOtp::where('bid_id', $bidId)
    ->where('user_id', $sellerId)
    ->first();

if ($existingOtp) {
    $existingOtp->delete(); // Delete the existing OTP
}

// Create a new OTP record
P2pWithdrawOtp::create([
    'bid_id' => $bidId,
    'user_id' => $sellerId,
    'email' => $email,
    'otp_code' => $code,
    ...
    'is_used' => false,
        ...
        try {
            ...
            $email_template_content = str_ireplace("{code}", $code, $email_template_content);
            ...
            dispatch(new \App\Jobs\SendWithdrawCodeEmailJob($data))->onQueue('forgot_password_email');
    ...
    public function verifyWithdrawOtp(Request $request)
    {
        ...
        $otp = P2pWithdrawOtp::where('bid_id', $bid_id)
    ...
    public function index(Request $request, $domain = '',$vendor,$url_slug ,$bid_type = null)
    {
        ...
        $client = Client::first();
        $urlbidid = $request->query('url_bid_id');
        ...
        elseif($product->category->categoryDetail->type_id == 7)
        {
            ...
            if (auth()->check()) { // Check if the user is logged in
                $user = auth()->user();
                $checkproductBid = P2pBid::with(['bidDocument','bidMilestones','buyer'])->where('product_id',$product->id)->where('buyer_id', $user->id)->where('id', $urlbidid)->first();
             
                $sellerIncomingBid = P2pBid::with(['bidMilestones','seller'])->where('product_id',$product->id)->where('seller_id', $user->id)->where('id', $urlbidid)->first();
            }else{
                $checkproductBid = P2pBid::with(['bidDocument','bidMilestones'])->where('product_id',$product->id)->where('id', $urlbidid)->first();
                $sellerIncomingBid = P2pBid::with('bidMilestones')->where('product_id',$product->id)->where('id', $urlbidid)->first();
            }
            
            $bidDocuments = P2pBidDocument::with('bidData.product')
             ->whereHas('bidData', function ($query) use($urlbidid) {
                $query->where('bid_status', 'matched')->where('id', $urlbidid); // Filter by bid_status
            })  
            ->whereHas('bidData.product', function ($query) use ($product) {    
            $query->where('id', $product->id); // Filter by product ID
            })->get();
        
              
            $soldOutProduct = p2pBid::where('product_id', $product->id)->where('bid_status', 'closed')->first();
            $vendorDetail = User::where('id', $product->vendor_id)->first();
    ...
    {
        if (!Auth::check()) {
            return response()->json(['status' => 'error', 'message' => 'Please login to add favorites.'], 401);
        }
    ...
    public function uploadDocCentre(Request $request){
            ...
            if ($request->hasFile('doc_1')) {
                ...
                if ($img->id > 0) {
                    ...
                    $image->is_default = 2;
                    $image->document_info= $request->info_one;
            ...
            if ($request->hasFile('doc_2')) {
                ...
                if ($img->id > 0) {
                    ...
                    $image->product_id = $request->product_id;
                    $image->is_default = 2;
                    $image->document_info= $request->info_two;
                    $image->document_type = 1;
                    $image->media_id = $img->id;
                    $image->save();
                }
