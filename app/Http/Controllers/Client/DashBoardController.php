...
use App\Models\{Banner, Brand, Category, Country, Order, Product, Vendor, VendorOrderStatus, UserAddress, OrderVendor, OrderReturnRequest, User, ClientCurrency, OrderNotificationsLogs, OrderVendorProduct, ServiceArea, UserVendor, Newsletter};
...
class DashBoardController extends BaseController
{
    ...
    public function postFilterDataNew(Request $request)
    {
        try {
            ...
            $total_products = $total_products->where('deleted_at', NULL)->count();
            $subscribed_user = Newsletter::count();
            ...
            if (Auth::user()->is_superadmin == 0) {
                ...
                if(count($vendorIds)>0)
                {
                    ...
                    ->whereHas('orderStatusVendor', function ($q) {
                        $q->where('order_status_option_id', 6)->where('order_status_option_id', '!=', '3');
                    });
            ...
            // Total Placed Company 
            $total_placed_company = P2PBid::where('bid_status', 'closed')
                ->where('withdraw_by', 0)
                ->where('confirm_ownership_transfer', 1)
                ->count();
            // Total Placed Company  Stopped

        //   \Log::info(['total sold company' =>  $total_placed_company ]);
            ...
            $countDraftedProductList = Product::where('is_live',0)->whereNull('deleted_at')->count();
            $response = [
                'markers' => $markers,
                'total_revenue' => round($total_revenue),
                'drafted_product_count' => $countDraftedProductList,
                'total_products' => $total_products,
                'total_customers' => $total_customers,
                'subscribed_users' => $subscribed_user,
                // 'placed_company' => $total_placed_company,
                ...
                'total_sold_products' => $total_placed_company ?? 0,
