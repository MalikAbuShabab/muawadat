...
class ClientPreferenceController extends BaseController{
    ...
    public function showSupportTicketList(Request $request){
        return view('backend/support/index');
    }

    public function fetechSupportData(Request $request)
    {
        if ($request->ajax()) {
            $tickets = SupportTicket::with('user')->select('support_tickets.*')->orderBy('id', 'DESC');
            return DataTables::of($tickets)
                ->addColumn('user', function ($ticket) {
                    return $ticket->user ? ucfirst($ticket->user->name) : 'N/A';
                })
                ->addColumn('subject', function ($ticket) {
                    return $ticket->subject ? $ticket->subject : 'N/A';
                })
                ->addColumn('message', function ($ticket) {
                    return $ticket->message ? $ticket->message : 'N/A';
                })
                ...
                ->addColumn('status', function ($bid) {
                    ...
                    foreach ($statuses as $status) {
                        $selected = $bid->status == $status ? 'selected' : '';
                ...
                ->addColumn('action', function ($ticket) {
                    return '<a href="#" data-id="' . $ticket->id . '" class="btn btn-info btn-sm view-support-ticket">View</a>
    ...
    public function showSupportDetail(Request $request){
        try{
    ...
    public function supportTicketdestroy($domain, $id){
    ...
    {
        $bid = SupportTicket::find($request->bid_id);
        if ($bid) {
   ...
   // enable/disable price key in Role table (START)
    ...
    public function update(Request $request, $code){
        ...
        if($request->has('social_login') && $request->social_login == '1'){
            ...
            $preference->nafath_login = ($request->has('nafath_login') && $request->nafath_login == 'on') ? 1 : 0;
        }
        ...
        if($request->has('total_payment_milestone') && $request->total_payment_milestone != '0'){
            $preference->total_payment_milestone = $request->total_payment_milestone ?? 1;
        }
