...
class OrderController extends BaseController
{
        ...
        try {
            ...
            if (!$vendor_order_status_check) {
                ...
                if ($request->status_option_id == 6) {
                    $user = User::whereHas('orders', fn ($q) => $q->where('id', $request->order_id))
                        ->with('referral')
                        ->first();

                    (new \App\Services\Referral($user->referral))->redeem();
                }
    ...
    {
        $order_dispatchs = 2;
        $AdditionalPreference  =  getAdditionalPreference(['is_place_order_delivery_zero','is_vendor_own_driver']);
        $checkdeliveryFeeAdded = OrderVendor::with('LuxuryOption', 'products','vendor')->where(['order_id' => $request->order_id, 'vendor_id' => $request->vendor_id])->first();
        ...
        $is_vendor_own_driver = $checkdeliveryFeeAdded->vendor->is_vendor_own_driver??0;
        $is_vendor_own_driver_config = $AdditionalPreference['is_vendor_own_driver']??0;

        //run only if vendor has own driver request is not gone to our dispatcher panel
        if($is_vendor_own_driver_config && $is_vendor_own_driver)
        {
            return 1;
        }
