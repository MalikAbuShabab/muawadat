...
<div class="col-md-4">
    <div class="form-group d-flex justify-content-between mb-3 alCustomToggleColor">
        <label for="is_vendor_own_driver_switch" class="mr-2 mb-0">{{ __('Own Driver') }}<small
                class="d-block pr-5">{{ __('Is Vendor Own Driver') }}</small></label>
        <span> <input type="checkbox" data-plugin="switchery" name="is_vendor_own_driver"
                id="is_vendor_own_driver_switch" class="form-control checkbox_change"
                data-className="is_vendor_own_driver" data-color="#43bee1"
                @if (getAdditionalPreference(['is_vendor_own_driver'])['is_vendor_own_driver']) checked='checked' @endif>
        </span>
        <input type="hidden" @if (getAdditionalPreference(['is_vendor_own_driver'])['is_vendor_own_driver'] == 1) value="1" @else value="0" @endif
            name="is_vendor_own_driver" id="is_vendor_own_driver" />
    </div>
</div>
