...
Route::group(['middleware' => 'adminLanguageSwitch'], function () {
    Route::group(['middleware' => ['ClientAuth', 'database', 'permission'], 'prefix' => '/client'], function () {
        ...
        Route::get('/admin/notifications/unread', [NotificationController::class, 'getUnreadNotifications'])->name('admin.notifications.unread');
        Route::get('/user/notifications/unread', [NotificationController::class, 'getUserUnreadNotifications'])->name('user.notifications.unread');
        ...
        // P2P BID CLIENT ROUTES //
            Route::get('p2p/admin/draft-product/show', 'Client\ProductController@showDraftedProducts')->name('p2p.admin.draft-product.list.show');
            Route::get('p2p/admin/draft-product/list', 'Client\ProductController@AllProductList')->name('p2p.admin.draft-product.list');
            Route::post('p2p/products/bulk-update-status', 'Client\ProductController@bulkUpdateStatus')->name('p2p.product.bulkUpdateStatus');
            Route::post('p2p/products/{id}/update-islive-status', 'Client\ProductController@updateProductIsLiveStatus')->name('p2p.product.update-islive-status');
            Route::get('p2p/admin/product/document/{id}', 'Client\ProductController@getDocCentre')->name('p2p.admin.product.document.show');
            
 
            Route::get('p2p/admin/bid/list', [VendorBidController::class, 'bidList'])->name('p2p.admin.bid.list');
            Route::get('p2p/admin/bid/fetchList', [VendorBidController::class, 'fetchList'])->name('p2p.admin.bid.fetchList');
            Route::post('p2p/admin/bid/status-update', [VendorBidController::class, 'updateStatus'])->name('p2p.bids.status-update');
            Route::get('p2p/admin/newsletter-subscribers', 'Front\NewsletterController@showNewsletterSubscribers')->name('p2p.admin.newsletter.subscribers');

            
            Route::get('p2p/support/list', 'Client\ClientPreferenceController@showSupportTicketList')->name('p2p.support.list');
            
            Route::get('p2p/support/datatable', 'Client\ClientPreferenceController@fetechSupportData')->name('p2p.support.datatable');
            Route::get('p2p/support-tickets', 'Client\ClientPreferenceController@showSupportDetail')->name('p2p.support_tickets.detail');
            Route::delete('p2p//support/ticket/delete/{id}', 'Client\ClientPreferenceController@supportTicketdestroy')->name('p2p.support.ticket.delete');

            Route::post('p2p/admin/support-tickets/update',   'Client\ClientPreferenceController@updateSupportStatus')->name('p2p.support.ticket-update');
        ...
        Route::get('user/list', 'Client\UserController@userList')->name('user.list');
        ...
        Route::post('identity/verifed/update/{id}', 'Client\UserController@userIdentityVerifed')->name('customer.identity.verifed.update');
