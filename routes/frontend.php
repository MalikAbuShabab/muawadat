...
Route::get('/contact-us', 'HomeController@contactUs')->name('contact-us');
	Route::post('/contact-us', 'HomeController@storeContactInquiry')->name('contact.store');
	////check Shiprocket
	...
	// Route::get('payment/dpo/success', 'Front\DpoController@successPage')->name('dpo.success');
	// Route::get('payment/dpo/fail', 'Front\DpoController@failPage')->name('dpo.fail');
	Route::get('/user-home-search/{slug?}', 'Front\UserhomeController@userHomeSearch')->name('user.home.search');
	Route::get('/all-categories-list', 'Front\UserhomeController@allCategories')->name('all.categories.list');
	Route::get('/filter-search/{sortField?}/{sortOrder?}', 'Front\UserhomeController@search')->name('filter.search');
	...
	Route::get('/', 'Front\UserhomeController@landingPage')->name('userLandingPage');

	//USER HOME CONTORLLER
	Route::post('/subscribe', 'Front\NewsletterController@subscribe')->name('newsletter.subscribe');
	...
	//SEND OTP ON FORGET PASSWORD
	Route::post('user/forgetPasswordOtp', 'Front\ForgotPasswordController@forgetPasswordOtp')->name('customer.forgotPass.otp');
	Route::get('user/forgetPasswordOtpForm', 'Front\ForgotPasswordController@forgetPasswordOtpForm')->name('customer.forgotPass.otp.form');
	Route::post('user/verify-resetPassword-otp', 'Front\ForgotPasswordController@verifyForgetPasswordOtp')->name('customer.verify.forgotPass.otp');
	// Route::get('reset-password', 'Front\ForgotPasswordController@getResetPasswordForm')->name('reset-password-link');
	Route::post('user/resetPassword', 'Front\CustomerAuthController@resetPassword')->name('customer.resetPass');
	Route::get('reset-password/{token}', 'Front\ForgotPasswordController@getResetPasswordForm')->name('reset-password-link');
	...
	Route::get('{vendor?}/product-page/{id?}/{slug1?}/{slug2?}', 'Front\ProductController@index')->name('productDetailPage');
...
Route::group(['middleware' => ['domain', 'webAuth']], function () {
	Route::get('authdashboard', 'Front\UserhomeController@index')->name('userHome');
	...
	Route::get('/user/two-factor-auth', 'Front\ProfileController@twoFactorAuth')->name('user.two-factor-auth');
	Route::get('/user/identity-proof', 'Front\ProfileController@userIdentityProofUpload')->name('user.identity-proof');
	Route::post('/user/identity-proof-store', 'Front\ProfileController@userIdentityProofStore')->name('user.identity-proof-store');
	Route::get('/user/money-verification', 'Front\ProfileController@moneyVerification')->name('user.money-verification');
	Route::get('/user/money-verification-invoice', 'Front\ProfileController@moneyVerificationInvoice')->name('user.money-verification-invoice');
	Route::get('/user/social-login-links', 'Front\ProfileController@socialLoginLinks')->name('user.social-login-links');
	Route::post('/user/social-login-links/store', 'Front\ProfileController@socialLoginLinksStore')->name('user.social-login-links-store');
	
	
	Route::post('/user/setup-2fa', 'Front\ProfileController@setupTwoFactorAuth')->name('user.setup-2fa');
	Route::post('/user/verify-2fa-otp', 'Front\ProfileController@verify2faOtp')->name('user.verify-2fa-otp');
	...
	Route::get('user/logout/landpage', 'Front\CustomerAuthController@logoutLandpage')->name('user.logout.landpage');
	...
	// P2P BID  ROUTES //
	Route::get('get-brand-categories/{id}',  'Front\PostController@getBrandCategories')->name('get.Brand.categories'); // BRAND = PROJEC TYPE
	Route::post('storePost', 'Front\PostController@storeCompanyData')->name("posts.storeCompanyData");
	Route::post('storeFinancialInfo', 'Front\PostController@storeCompanyFinancialInfo')->name("posts.storeCompanyFinancialInfo");
	Route::post('uploadCompanyDoc', 'Front\PostController@uploadCompanyDoc')->name("posts.uploadCompanyDoc");
	Route::get('listing-documents', 'Front\PostController@listingFormDocument')->name('listing-documents');
	Route::get('p2p/bidList', 'Front\BuyerBidController@index')->name("p2p.bid.list");
	 
	Route::post('p2p/store/raisebid', 'Front\BuyerBidController@storeRaisedBidByBuyer')->name("p2p.raisebid.store");
	Route::post('marketplace/raised-bid-amount', 'Front\BuyerBidController@raisedBidAmount')->name("p2p.raised-bid-amount");

	Route::post('p2p/upload-signature', 'Front\BuyerBidController@uploadBidSignaure')->name("p2p.upload.signature");
	Route::get('p2p/view-buyer-contracts', 'Front\BuyerBidController@viewBuyerBidContract')->name("p2p.view-buyer-contracts");
	Route::get('p2p/download-signed-agreement/{id}', 'Front\BuyerBidController@downloadContractSign');
	// Route::get('/user-home-search', 'Front\UserhomeController@userHomeSearch')->name('user.home.search');

	// Route::get('/filter-search/{sortField?}/{sortOrder?}', 'Front\UserhomeController@search')->name('filter.search');

	Route::get('p2p/marketplace', 'Front\SellerBidController@index')->name("p2p.marketpalce.list");
	Route::post('p2p/send-withdraw-verification-code', 'Front\ProductController@sendWithdrawCode')->name('p2p.send-withdraw-verification-code');
	Route::post('/p2p/verify-withdraw-otp',  'Front\ProductController@verifyWithdrawOtp')->name('p2p.verify-withdraw-otp');
	Route::get('marketplace/update-bid-status', 'Front\SellerBidController@updateBidStatus')->name("marketpalce.update-bid-status");
	Route::post('marketplace/reject-raised-bid', 'Front\SellerBidController@requestRaisedBid')->name("marketpalce.reject-raised-bid");
	Route::post('marketplace/update-signature', 'Front\SellerBidController@uploadSignature')->name("marketpalce.upload.signature");
	Route::get('marketplace/view-contracts', 'Front\SellerBidController@viewBidContract')->name("marketpalce.view-contracts");
	Route::post('marketplace/ownership-confirm', 'Front\SellerBidController@ownerShipTransferConfirmed')->name('marketpalce.ownership-confirm');
	Route::post('p2p/upload/doc-centre', 'Front\ProductController@uploadDocCentre')->name('p2p.documents.upload');
	

	Route::get('marketplace/seller-bid-count', 'Front\SellerBidController@getSellerBidCount')->name('marketpalce.seller-bid-count');
	Route::post('marketplace/mark-bid-as-viewed', 'Front\SellerBidController@markBidsAsViewed')->name('marketpalce.mark-bid-as-viewed');

	
	Route::post('marketplace/milestone-store', 'Front\PaymentMilestoneController@store')->name("marketplace.store-milestone");
	Route::post('marketplace/milestone-update', 'Front\PaymentMilestoneController@update')->name("marketplace.update-milestone");
	Route::get('marketplace/milestone-list', 'Front\PaymentMilestoneController@list')->name("marketplace.list-milestone");
	Route::post('toggle-favorite', 'Front\ProductController@toggleFavorite')->name('toggle.favorite');
	
	Route::get('p2p/buyer-milestone-list', 'Front\PaymentMilestoneController@buyerMilstoneViewlist')->name("buyer.list-milestone");
	Route::put('p2p/{milestone}/toggle-approval',  'Front\PaymentMilestoneController@toggleApproval')->name('p2p.milestone.toggleApproval');

	Route::post('marketplace/payment/mark-complete',  'Front\PaymentMilestoneController@updateMarkPaymentStatus')->name('marketplace.payment.mark-complete');
 
	//Both seller or buyer
	Route::post('p2p/store/withdraw-reason',  'Front\SellerBidController@storeWithdrawReason')->name('p2p.store.withdraw-reason');
	
	// Rating
	Route::post('marketpalce/rating/save', 'Front\P2pRatingController@store')->name('marketpalce.rating.save');
	// P2P SUPPORT 
	Route::get('customer/support/form', 'Front\SupportTicketController@customerSupportForm')->name('customer.support.form');
	Route::get('customer/support/tickes', 'Front\SupportTicketController@customerSupportTickets')->name('customer.support.tickets');
	Route::post('customer/support/store', 'Front\SupportTicketController@store')->name('customer.support.store');
